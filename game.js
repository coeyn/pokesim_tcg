...
function updateBattleStats() {
  ...
  if (e.statPlayerDeck) e.statPlayerDeck.textContent = `${gt('ui.deckZoneLabel')}: ${playerDeck}`;
  if (e.statPlayerHand) e.statPlayerHand.textContent = `${gt('ui.handTitle')}: ${playerHand}`;
  if (e.statOppDeck) e.statOppDeck.textContent = `${gt('ui.deckZoneLabel')}: ${opponentDeck === null ? '-' : opponentDeck}`;
  if (e.statOppHand) e.statOppHand.textContent = `${gt('ui.handTitle')}: ${opponentHand === null ? '-' : opponentHand}`;
}
