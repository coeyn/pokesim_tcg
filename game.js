...
function mFinish(x, y) { if (!S.mDrag) return; let sharedChanged = false; const ob = inR(x, y, e.bag.getBoundingClientRect()), op = inR(x, y, e.playmat.getBoundingClientRect()); if (S.mDrag.src === 'catalog' && op) { const p = { x: Math.max(16, Math.min(e.playmat.clientWidth - 16, x - e.playmat.getBoundingClientRect().left)), y: Math.max(16, Math.min(e.playmat.clientHeight - 16, y - e.playmat.getBoundingClientRect().top)) }; S.markers.push({ id: S.nextMarker++, type: S.mDrag.type, x: p.x, y: p.y }); renderMarkers(); sharedChanged = true } if (S.mDrag.src === 'board') { const i = S.markers.findIndex(m => m.id === S.mDrag.id); if (i >= 0) { if (ob) { S.markers.splice(i, 1); sharedChanged = true } else if (op) { const p = { x: Math.max(16, Math.min(e.playmat.clientWidth - 16, x - e.playmat.getBoundingClientRect().left)), y: Math.max(16, Math.min(e.playmat.clientHeight - 16, y - e.playmat.getBoundingClientRect().top)) }; S.markers[i].x = p.x; S.markers[i].y = p.y; sharedChanged = true } renderMarkers() } } if (sharedChanged) saveNow() }
